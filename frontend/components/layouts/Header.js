...
import { Menu, Bell, X, User, Search, ShoppingCart, BookOpen, CreditCard, LogOut, ChevronDown } from "lucide-react";
...
const Header = ({ onNavigate = () => {}, onSubscriptionClick = () => {}, onSignIn = () => {}, onSignUp = () => {}, notificationCount = 3 }) => {
	...
	// Sync with navigationState when component mounts
	useEffect(() => {
	...
	const handleSubscriptionClick = () => {
		...
		if (!user) {
			// Update navigation state to show modal
			navigationState.showAuthModal = true;
			navigationState.attemptedNavigation = "Subscription";
			return;
		}
					...
					<div className="profile-panel-content">
						...
						{user ? (
							<>
								...
								{/* Navigation Menu - Updated to use the new navigation pattern */}
								<div className="profile-panel-section">
								...
								{/* Publisher Section */}
								{user &&
									(user.accountType?.includes("Publisher") ? (
										<div className="profile-panel-section publisher-section">
											<div className="section-label">Publisher Dashboard</div>
											<button
										...
										<div className="profile-panel-section publisher-section">
											...
											<button
												className="profile-panel-btn warning"
												onClick={() => {
													setShowProfilePanel(false);
													router.push("/auth/author");
												}}
											>
...
Header.detectUserCurrency = detectUserCurrency;
Header.isValidCurrency = isValidCurrency;
Header.getSupportedCurrencies = getSupportedCurrencies;
Header.CURRENCY_RATES = CURRENCY_RATES;
Header.CURRENCY_SYMBOLS = CURRENCY_SYMBOLS;
