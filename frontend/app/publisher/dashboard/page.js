		...
		<div className="landing-container">
			...
			<section className="category-selection-section">
				<div className="container">
					...
					{/* Dropdown View */}
					{showDropdown ? (
						<CategoryDropdown categories={categories} activeCategory={activeCategory} onCategorySelect={handleCategorySelect} className="mb-4" />
					) : (
						/* Horizontal Category Tabs - Original code */
						<div className="horizontal-category-tabs">
							{displayCategories.length > 0 ? (
								<div className="tabs-scroll-container">
									{displayCategories.map((category) => (
										<CategoryTab key={category} category={category} isActive={activeCategory === category} onClick={() => handleCategorySelect(category)} />
									))}

									{/* Show More/Less button for mobile */}
									{isMobile && categories && categories.length > 6 && (
										<button className="category-tab show-more-btn" onClick={() => setShowAllCategories(!showAllCategories)}>
								...
								<div className="horizontal-scroll">
									{topSellingBooks.map((book) => (
										<div key={book.id} className="book-scroll-item">
											<BookCard book={book} currency={userCurrency} onBuyNow={handleBuyNow} onAddToCart={handleAddToCart} />
										</div>
									))}
								</div>
							</div>
